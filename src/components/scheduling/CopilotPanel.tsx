...
const CopilotPanel: React.FC = () => {
    ...
    const fetchRatios = async () => {
        ...
        const { data } = await supabase.from('staffing_ratios').select('*').eq('organization_id', organization.id);
    ...
    const handleGenerate = () => {
        ...
        const activeRatios = ratios.length > 0 ? ratios : [
            { id: '1', organization_id: organization.id, zone_name: 'Daycare', staff_count: 1, dog_count: 15 },
            { id: '2', organization_id: organization.id, zone_name: 'Boarding', staff_count: 1, dog_count: 30 }
        ];
