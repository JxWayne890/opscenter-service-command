...
import { ChevronLeft, ChevronRight, Plus, Calendar as CalendarIcon, Users, Filter, Save } from 'lucide-react';
...
const ScheduleView = () => {
    const { shifts, staff, publishSchedule, updateShift } = useOpsCenter();
    ...
    const handleDragStart = (e: React.DragEvent, shiftId: string) => {
        ...
        // Set transparent image or data if needed
    };
                                ...
                                {mobileShifts.filter(s => s.is_open).map(shift => (
                                    <div key={shift.id} onClick={(e) => handleEditShift(shift, e)} className="bg-white border-2 border-slate-100 rounded-xl p-4 flex justify-between items-center shadow-sm active:scale-[0.99] transition-transform">
                                        <div>
                                            ...
                                            <div className="text-xs text-slate-500 font-medium">{shift.role}</div>
                                        </div>
                        ...
                        <div className="grid grid-cols-8 divide-x divide-slate-50 group hover:bg-slate-50/50 transition-colors">
                            ...
                            {weekDates.map((date, i) => {
                                ...
                                return (
                                    <div
                                        key={i}
                                        className="p-2 min-h-[100px] relative transition-colors hover:bg-white/80"
                                        ...
                                        <div className="space-y-2">
                                            {dayShifts.map(shift => (
                                                <div
                                                    ...
                                                    className="bg-white border-2 border-dashed border-slate-300 p-2 rounded-lg cursor-grab active:cursor-grabbing hover:border-indigo-400 hover:shadow-md transition-all group/shift"
                                                >
                                                    ...
                                                    <div className="text-xs font-bold text-slate-700">{shift.role}</div>
                                                </div>
                                            ))}
                                            <button
                                                onClick={() => handleNewShift(date)}
                                                className="w-full h-8 flex items-center justify-center rounded-lg border-2 border-transparent hover:border-dashed hover:border-slate-300 text-transparent hover:text-slate-400 transition-all"
                                            >
                                                <Plus size={16} />
                                            </button>
                                        </div>
                                            ...
                                            <div className="space-y-2">
                                                {dayShifts.map(shift => (
                                                    <div
                                                        ...
                                                        onDragStart={(e) => handleDragStart(e, shift.id)}
                                                        onClick={(e) => handleEditShift(shift, e)}
                                                        className={`p-2 rounded-lg cursor-grab active:cursor-grabbing shadow-sm border border-transparent hover:shadow-md transition-all ${shift.is_open ? 'bg-slate-100' : 'bg-emerald-50 border-emerald-100 text-emerald-900'}`}
                                                    >
                                                        ...
                                                        <div className="text-xs font-bold leading-tight">{shift.role}</div>
                                                    </div>
                                                ))}
                                                <button
                                                    onClick={() => handleNewShift(date, user.id)}
                                                    className="w-full h-full absolute inset-0 opacity-0 hover:opacity-100 flex items-center justify-center pointer-events-none group-hover:pointer-events-auto"
                                                >
                                                    <div className="w-6 h-6 bg-indigo-50 text-indigo-500 rounded-full flex items-center justify-center shadow-sm pointer-events-auto cursor-pointer transistion-transform hover:scale-110">
                                                        <Plus size={14} />
                                                    </div>
                                                </button>
                                            </div>
