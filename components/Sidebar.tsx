...
const SidebarIcon = ({ icon: Icon, active, onClick }: { icon: any, active?: boolean, onClick: () => void }) => (
    ...
    >
        {active && (
            <div className="absolute inset-0 bg-brand-blue/20 rounded-2xl blur-xl animate-pulse" />
        )}
        <div className={`relative z-10 p-1 rounded-xl transition-all duration-300 ${active ? 'bg-brand-blue shadow-lg shadow-brand-blue/40 ring-1 ring-white/20 scale-110' : 'group-hover:bg-white/5'}`}>
    ...
    return (
        <aside className="hidden lg:flex flex-col glass w-20 rounded-[2rem] items-center py-8 space-y-8 sticky top-6 h-[calc(100vh-3rem)]">
            ...
            <div className="flex-1 flex flex-col space-y-4">
                ...
                {/* Roster - Manager only sees full roster, staff sees their profile */}
                ...
                {canManage && <SidebarIcon icon={Send} active={activeView === 'comms'} onClick={() => setActiveView('comms')} />}
                {canManage && <SidebarIcon icon={Settings} active={activeView === 'settings'} onClick={() => setActiveView('settings')} />}
            </div>
        ...
        <>
            ...
            {isMenuOpen && (
                <div className="fixed bottom-24 right-4 z-[90] glass-dark rounded-2xl p-4 flex flex-col space-y-4 animate-in slide-in-from-bottom-10 fade-in duration-300 min-w-[160px] shadow-2xl border border-white/10">
                    <button onClick={() => { setActiveView('roster'); setIsMenuOpen(false); }} className={`flex items-center space-x-3 p-3 rounded-xl transition-all ${activeView === 'roster' ? 'bg-brand-blue text-white' : 'text-slate-300 hover:bg-white/10'}`}>
                        <Users size={18} />
                        <span className="text-sm font-bold">{canManage ? 'Roster' : 'My Profile'}</span>
                    </button>
                    ...
                    <div className="h-px bg-white/10 my-1" />
                    <button onClick={logout} className="flex items-center space-x-3 p-3 rounded-xl text-rose-400 hover:bg-rose-500/10 transition-all w-full">
                        <LogOut size={18} />
                ...
                <nav className="relative flex items-center justify-between p-2 h-20 bg-slate-950/90 backdrop-blur-2xl rounded-3xl border border-white/10 shadow-[0_20px_50px_rgba(0,0,0,0.5)] ring-1 ring-white/5">
                    ...
                    <div className="relative -mt-6">
                        ...
                        <button
                            ...
                            className="relative group w-16 h-16 bg-gradient-to-tr from-brand-blue via-brand-dark to-brand-blue rounded-[20px] flex items-center justify-center text-white shadow-[0_0_25px_rgba(0,174,239,0.5)] active:scale-90 transition-all duration-500 border border-white/20 overflow-hidden"
                        >
